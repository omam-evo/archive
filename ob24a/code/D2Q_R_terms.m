function [D2Q, dD2Q_ds] = D2Q_R_terms(A, ALPHA, sigma, R, N)
    T1 = 1;
    T2 = 0;
    ZERO = 0;
    f = @(s,r,a,freq,n) 2*n*s.^4 + 4*r.^2.*s.^2 + ...
            T1*n*a^2/2 * (1-ZERO*exp(-(freq*s).^2)) .* (1-ZERO*exp(-freq^2*(s.^2 + 2*(r/sqrt(n)).^2))) + ...
            T2*2*n*a*freq^2*s.^2 .* exp(-0.5*freq^2*(s.^2+(r/sqrt(n)).^2)) .* (s.^2 + 2*(r/sqrt(n)).^2);
