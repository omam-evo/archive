classdef EvalDyn 
    methods     (Static=true)
        ...
        function [] = dyn_avg(r_avg, f_avg, sigma_avg, sigma_norm_avg, flag_success, num_gen, OPTIONS)
            ...
            if any(strcmpi(OPTIONS, 'NEWFIG')), figure; end
                hold on;
                g=0:1:N_G; 
                    plot(g, r_avg, 'k-', 'DisplayName', '$R$: avg');
                    if any(strcmpi(OPTIONS, 'SIG'))
                        plot(g, sigma_avg, 'r-', 'DisplayName', '$\sigma$');   
                        plot(g, sigma_norm_avg, 'b-', 'DisplayName', '$\sigma^*$');
                    end
                    if any(strcmpi(OPTIONS, 'FIT'))
                        plot(g, f_avg, 'g-', 'DisplayName', '$f$');
                    end
            if any(strcmpi(OPTIONS, 'NEWFIG'))
                set(gca, 'YScale', 'log');
        ...
        function [] = dyn_avg_glob_attr(r_g_t, flag_success)  
            ...
            for i=1:num_success
                ...
                d(i) = c(2);
                if PLOT_ALL_LIN==1
                    px = linspace((pow_target-d(i))/k(i), x(end), 101);
                    py = k(i)*px + d(i);
                    plot(px, 10.^py, 'b:'); 
                end
        ...
        function [] = dyn_avg_btw_target(r_g_t, flag_success, num_intervals, OPTIONS)  
            ...
            if any(strcmpi(OPTIONS,'mean'))
                ...
                color = 'm.'; type = 'mean';
            elseif any(strcmpi(OPTIONS,'median'))
                g_avg = nan*zeros(num_intervals, 1);
                for j=1:num_intervals
                    gen_list = [];
                    pow_r_low = pow_range(j);    % get interval
                    pow_r_high = pow_range(j+1); % get interval
                    for i=1:num_success
                        get = pow_r_low<=log_r_g_t(:,i) & log_r_g_t(:,i)<=pow_r_high;   % get ids, such that value btw. [pow_R(j)+pow_R(j+1)]
                        gen_list = [gen_list;gen(get)];                                  % get generation values in a list
                    end 
                    g_avg(j) = median(gen_list);
                end   
                color = 'g.'; type = 'median';
            else
            ...
            %% Plot avg.     
            plot(g_avg, 10.^pow_avg, color, 'DisplayName', [type '($g$ in $[R,R\cdot\delta R]$)']);
            if any(strcmpi(OPTIONS, 'debug'))
                legend('AutoUpdate','off');
                plot(repmat([gen(1,1),gen(end,1)],length(pow_range),1)',[10.^pow_range,10.^pow_range]', ':', 'color', [0.6,0.6,0.6]);
                legend('AutoUpdate','on');
            end
        end%function
        function [] = histogram_R(r_g_t, gen_val, OPTIONS)
            id_gen = gen_val+1;
            figure; hold on; title(['Generation $g$=',num2str(gen_val)]); ylabel('Count')
            if any(strcmpi(OPTIONS, 'lin'))
                R_at_g = r_g_t(id_gen, :);
                xlabel('Distr. of $R$');
            elseif any(strcmpi(OPTIONS, 'log'))
                R_at_g = log10(r_g_t(id_gen, :));
                xlabel('Distr. of log10($R$)');
            end
            histogram(R_at_g);
            myfigstyle(gcf, 16, 10, 10, 10);
            fprintf('mean(R)=%d, median(R)=%d \n',mean(R_at_g),median(R_at_g));
        end
        function [] = histogram_gen(gen_vec, r_g_t, R_lb, R_ub)
            
            assert(R_lb<R_ub);
            gen_vec = reshape(gen_vec,length(gen_vec),1); % to be sure shape is ok         
            num_trials = size(r_g_t,2);
            
            gen_list = [];
            for i=1:num_trials
                get = R_lb<=r_g_t(:,i) & r_g_t(:,i)<=R_ub;   % get ids, such that value btw.
                gen_list = [gen_list;gen_vec(get)];
            end
            figure; hold on; title(['$R$ in [',num2str(R_lb),',',num2str(R_ub),']']); 
            xlabel('Distr.~of $g$'); ylabel('Count');
            histogram(gen_list);
            myfigstyle(gcf, 16, 10, 10, 10);
            fprintf('mean(g)=%d, median(g)=%d \n',mean(gen_list),median(gen_list));
        end
    end %methods
end
