classdef EvalPhi
    methods     (Static=true)
            ...
            for i=1:NUM_POS
                ...
                if any(strcmpi(OPTIONS, 'PLOT'))
                    ...
                    if any(strcmpi(OPTIONS, 'SPH'))
                        plot(SIGMA_LIST, phi_R_SPH(:,i), ':', 'color', [0.5,0.5,0.5], 'DisplayName', '$\varphi^*_{\mathrm{sph}}$');
                    end
                end
        ...
        %% Old Phi_v1
        init = nan*zeros(NUM_SIG, NUM_POS);
        [phi_1_num,phi_i_num_A1,phi_i_A2,phi_i_num_A2] = ...
            deal(init,init,init,init);
        ...
        for p=1:NUM_POS
            ...
            for s=1:NUM_SIG
                ...
                %[phi, ~, ~] = Phi.get_phi_1_viaC_Di(ras_tools, C_MU_LAM, A, ALPHA, sigma, Y_0, 1);
                %phi_1_C_f(:, s, p) = phi;
                [phi, ~, ~] = p2.get_phi_1(ras_tools, MU, LAM, A, ALPHA, sigma, Y_0, 1);
                phi_1_A3(:, s, p) = phi;
                
                if any(strcmpi(OPTIONS, 'NUM')) % show numeric  
                    phi_1_num(s,p) = p2.get_phi_1_num(ras_tools, A, ALPHA, MU, LAM, sigma, Y_0, Y_i, Y_jni);
                end   
                
                %% Old Phi1 calculations via Phi_v1
                p1 = Phi_v1(MU, LAM, ras_tools, A, ALPHA, Y_0, Y_i, Y_jni, sigma, 'nonlinear');
                if any(strcmpi(OPTIONS, 'A1-NUM'))
                    [phi_i_num_A1(s,p), ~] = p1.get_phi_large_pop_num();
                end
                if any(strcmpi(OPTIONS, 'A2-NUM'))
                    [phi_i_num_A2(s,p), ~] = p1.get_phi_PhiAB_num();
                end
                if any(strcmpi(OPTIONS, 'A2'))
                    [phi_i_A2(s,p), ~] = p1.get_phi_PhiAB(ras_tools, A, ALPHA, Y_i);
                end
            end
        end
        ...
        for p=1:NUM_POS
            ...
            xlabel(str_sig_label); ylabel('$\varphi_i$');
                ...
                if any(strcmpi(OPTIONS, 'A1-NUM'))
                    plot(SIGMA_LIST, phi_i_num_A1(:, p), 'c:', 'DisplayName', 'A1-NUM');% = 2y_i\varphi_i - \sigma^2/\mu - [...]$');
                end
                if any(strcmpi(OPTIONS, 'A2'))
                    plot(SIGMA_LIST, phi_i_A2(:, p), '--', 'color', [0.9290, 0.6940, 0.1250], 'DisplayName', 'A2');% = 2y_i\varphi_i - \sigma^2/\mu - [...]$');
                end
                if any(strcmpi(OPTIONS, 'A2-NUM'))
                    plot(SIGMA_LIST, phi_i_num_A2(:, p), 'b:', 'DisplayName', 'A2-NUM');% = 2y_i\varphi_i - \sigma^2/\mu - [...]$');
                end
                if any(strcmpi(OPTIONS, 'A3'))
                    plot(SIGMA_LIST, phi_1_A3(COMP_ID, :, p), 'r-.', 'DisplayName', 'A3');% = 2y_i\varphi_i - \sigma^2/\mu - [...]$');
                end
