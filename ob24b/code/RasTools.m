classdef RasTools
   properties
    ...
    fac_sigma2norm;
    fac_q2norm;
    f_cos1; f_cos2; f_sin2; f_x2cos1; f_x1sin1; f_x1cos1; f_x3; f_x2sin2;
    EV_cos1; EV_cos2; EV_sin2; EV_x2cos1; EV_x1sin1; EV_x1cos1; EV_x3; VAR_cos1; VAR_sin1; EV_x2sin2;
   ...
   methods
       ...
       function [obj] = RasTools()
            ...
            %% R-dep
            obj.f_R = @(R, alpha, a, N) R^2 + N*a*(1 - exp(-0.5*(alpha*R)^2/N));
            ...
            obj.f_x2sin2 = @(a,x) x.^2 .* sin(a*x).^2;
            obj.EV_x2sin2 = @(a,mu,s) 0.5*((mu^2+s^2) - obj.EV_x2cos1(2*a,mu,s));           
    ...
    function [EQ_R] = expval_q_gain_R(obj, A, ALPHA, sigma, R, N)
        EQ_R = N*sigma^2+N*A*exp(-0.5*ALPHA^2*R^2/N)*(1-exp(-0.5*ALPHA^2*sigma^2));
    end
...
%             N*2*A*alpha*sigma^2*exp(-(alpha*sigma)^2/2)*(alpha*sigma^2*ev_cos1a + 2*ev_ysinay);
    ...
    %% Approx. D_Q with small R
    function [Var_q] = variance_q_gain_smallR(obj, A, ALPHA, R, N, sigma)    
        Var_q = (2+ALPHA^2*A)^2 * (sigma.^2 .* R.^2 + 0.5*N* sigma.^4);
    end
    %% Approx. D_Q with large R
    function [Var_q] = variance_q_gain_largeR(obj, A, ALPHA, R, N, sigma, OPTIONS)
        Var_q = 0;
        if any(strcmpi(OPTIONS,'R2')) Var_q = Var_q + 4*(sigma.^2).*(R.^2); end
        if any(strcmpi(OPTIONS,'NS4')) Var_q = Var_q + 2*N*sigma.^4; end
        if any(strcmpi(OPTIONS,'NA2')) Var_q = Var_q + N*A^2/2; end
    end
...
%             warning('0*2*sigma^4 0*4*sigma^2*Y(i)');
    ...
    %% Calculate derivatives of E_Q and D_Q
    ...
    %% MAIN RESULTS
    function [EQ, dEQ_ds] = EQ(~, A, ALPHA, sigma, Y)
    ...
    function [D2Q, dD2Q_ds] = D2Q(~, A, ALPHA, sigma, Y)
        fi = @(s,y,A,ALPHA) 2*s^4 + 4*Y.^2*s^2 + ...
            A^2/2*(1 - exp(-(ALPHA*s)^2))*(1-cos(2*ALPHA*Y)*exp(-(ALPHA*s)^2)) + ...
    ...
    %% E_Q(R) and derivative of E_Q(R)
    function [EQ, dEQ_ds] = EQ_Rdep(~, A, ALPHA, sigma, R, N)
        %syms r s freq a n
    ...
    %% D2_Q(R) and derivative of D2_Q(R)
    function [D2Q, dD2Q_ds] = D2Q_Rdep(~, A, ALPHA, sigma, R, N)
        %syms r s freq a n
