...
classdef Phi_v2
    methods 
        ...
        function[phi_2] = get_phi_2_hybrid_Y_R(obj, ras_tools, mu, A, ALPHA, sigma, Y, R, c_vartheta)

            N = length(Y);
                
            % --- Choose variance ---
%             D_Q = sqrt(sum(ras_tools.variance_q_gain_simplified(A, ALPHA, Y, sigma)));
            D_Q = sqrt(ras_tools.variance_q_gain_R(A, ALPHA, R, N, sigma)); % R-dep variance and Y-dep gain part => HYBRID
            
            %% Get Phi2
            phi_2 = c_vartheta * sigma^2 / D_Q * ...
                (4*Y.^2 + 2*ALPHA*A*exp(-(ALPHA*sigma)^2/2)*Y.*sin(ALPHA*Y)) ...
                - sigma^2 / mu;
        end
        
        %% Phi 2 as function of R
        function[phi_2_R, D_Q_R] = get_phi_2_R(~, ras_tools, mu, A, alpha, sigma, R, N, c_vartheta)
            D_Q_R = sqrt(ras_tools.variance_q_gain_R(A, alpha, R, N, sigma));
            phi_2_R = c_vartheta * sigma.^2 .* (2*R.^2) ./ D_Q_R  .* (2 + alpha^2*A*exp(-0.5*alpha^2*(sigma.^2+R.^2/N))) - N*sigma.^2/mu;
        ...
        %% Phi 2 as function of R
        function[phi_2_R] = TEST_FCT_get_phi_R(~, ras_tools, mu, A, alpha, sigma, R, N, c_vartheta)
            %% Full R-dep sol
%             D_Q_R = sqrt(ras_tools.variance_q_gain_R(A, alpha, R, N, sigma));
            %% Small R
%             D_Q_R = sqrt(ras_tools.variance_q_gain_smallR(A, alpha, R, N, sigma));
            %% Medium & large R (SEE OPTIONS 'R2','NS4','NA2')
            D_Q_R = sqrt(ras_tools.variance_q_gain_largeR(A, alpha, R, N, sigma, {'R2','NS4','NA2'}));
            FLAG_ZERO = 0;
            phi_2_R = c_vartheta * sigma.^2 .* (2*R.^2) ./ D_Q_R  .* (2 + FLAG_ZERO*alpha^2*A*exp(-0.5*alpha^2*(sigma.^2+R.^2/N))) - N*sigma.^2/mu;
        end
        
        %% TEST enviroment for phi_2 terms on/off etc.
        function[phi_2, T1_E2, T2_E2, T3_E11] = get_phi_2_lgPop(~, ras_tools, mu, lam, A, ALPHA, sigma, Y_0, OPTIONS)
...
%             phi_2 = 2*Y_0.*phi_1 - E2_overMuSq - 2*E11_overMuSq;
        ...
        function[phi_2_overNcomp] = phi_2_i_overNcomp(~, ras_tools, A, ALPHA, MU, LAM, sigma, Y_0, OPTIONS)
            N = length(Y_0);
        ...
        function[phi_2_num, phi_1_num_lgPop, E2_overMuSq, E11_overMuSq] = get_phi_2_num_lgPop(~, ras_tools, A, ALPHA, MU, LAM, sigma, Y_0, Y_i, Y_jni)
...
%             fprintf(' done.\n');    
        ...
        %% Get Progress Rate from experiment
        function[phi_2_mean, phi_2_stderr ] = phi_2_experiment(~, TRIALS, FIT, MU, LAM, Y_0, SIGMA_0)
            ...
            parfor t = 1:TRIALS
                ...
                [y_opt, F_opt_g, r_g, sigma_g, gen] = muComLam_sSA(FIT, N, MU, LAM, Y_0, Y_HAT, SIGMA_0, TAU, SIGMA_STOP, F_STOP, N_G, 0);

                %% Phi_1, Phi_2, Phi_R by simulation
        ...
        %% Get PHI and PSI from experiment
        function[phi_2_mean, phi_2_stderr, psi_mean, psi_stderr] = phi2_psi_experiment(~, TRIALS, FIT, MU, LAM, TAU, Y_0, SIGMA_0)

            N = length(Y_0);
            N_G = 1; SIGMA_STOP = nan; F_STOP = nan; Y_HAT=0*Y_0;
            phi_2_trial = zeros(N, TRIALS);
            psi_trial = zeros(1, TRIALS);

            parfor t = 1:TRIALS

                %R_0 = norm(Y_0 - Y_HAT);

                [y_opt, ~, ~, sigma_g, ~] = muComLam_sSA(FIT, N, MU, LAM, Y_0, Y_HAT, SIGMA_0, TAU, SIGMA_STOP, F_STOP, N_G, 0);

                %% Phi_1, Phi_2, Phi_R by simulation
                phi_2_trial(:, t) = Y_0(:).^2 - y_opt(:).^2;
                psi_trial(1, t) = (sigma_g(end) - SIGMA_0) / SIGMA_0;
            ...
            phi_2_stderr = std(phi_2_trial, 0, 2)/sqrt(TRIALS);  
            % ----- Psi -----
            psi_mean = mean(psi_trial);
            psi_stderr = std(psi_trial)/sqrt(TRIALS); 
        end 
        
        %% Phi 1 according to A3
        function[phi_1, ki, exp_di] = get_phi_1_tau(~, ras_tools, mu, lam, A, ALPHA, sigma, Y, TAU)
            D_Q = sqrt(sum(ras_tools.variance_q_gain_simplified(A, ALPHA, Y, sigma)));
            theta = mu/lam;
            c_theta = 1/sqrt(2*pi)/theta * exp(-1/2*(norminv(theta))^2);
            ki = 2*Y;
            exp_di = exp(-(ALPHA*sigma)^2/2)*ALPHA*A*sin(ALPHA*Y);
            phi_1 = c_theta * sigma^2 / D_Q * ki * (1+2*TAU^2);
            % if any(imag(phi_1) ~= 0), pause, end
        end
