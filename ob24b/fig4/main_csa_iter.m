...
%% Readme:
% > Switch config. of Fig 4a and 4b below (different parameter variation and CSA)
% > Switch approximation quality 1A, 1B, 2A, 2B, see details in paper, by (un-)commenting respective section below.
% > data points (steady-state simulations of real CSA-ES) displayed in Fig. 4 can be taken from Fig. 5

%% Fig 4a: 
LIST_CONFIG = [3];
C_handle = @(MU,N) 1/sqrt(N);   % @(MU,N) 1/N;
D_handle = @(MU,N) 1/C_handle(MU,N);  % @(MU,N) 1/C_handle(MU,N);
