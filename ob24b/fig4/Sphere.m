    ...
    methods     (Static=true)
        function [res] = phi(sigma_norm, C_MU_LAM, MU, LAM, N, OPTIONS)
            ...
            elseif any(strcmpi(OPTIONS,'large'))
                f(x) = C_MU_LAM*sqrt(2*N) - x^2/(2*MU);
            else
...
%             sigeps_norm(R) = N/(2*R^2) * sqrt(N*A^2*ras_tools.VAR_cos1(ALPHA, 0, R/sqrt(N)));
        ...
        function [] = phi_star(obj, MU, LAM, N)
            C_MU_LAM = e_mu_lam_a_b_v2(MU,LAM,1,0);
            [x_opt, x_2ndZero] = obj.signorm_opt_sph(C_MU_LAM, MU, LAM, N, {'medium'});
            sign = linspace(0,ceil(x_2ndZero),501);
            figure; hold on;
                plot(sign, obj.phi(sign, C_MU_LAM, MU, LAM, N, {'medium'}), 'k-');
                plot(x_2ndZero,0,'o');
        end

        function [res] = phi_arn(sign, MU, N, e_10, e_11, e_20)
            DQ = sqrt(1+sign.^2/(2*N));
            res = sign.*e_10./DQ - sign.^2/(2*MU) .* ...
                (1 + 1/N*(e_11+(MU-1)*e_20)./(DQ.^2) - (N-1)/N^2.*sign*e_10./DQ);
        end
    end % methods
end % class
