%% Source: http://www.cmap.polytechnique.fr/~nikolaus.hansen/cmaes_inmatlab.html
%

function [xmin]=purecmaes(strfitnessfct, mu, lambda, N, xmean, sigma, stopfitness, stopeval, SET_CMA_ES, SET_INTERMEDIATE, SET_MATRIX_NORM)
...
%   mu = lambda/2;               % number of parents/points for recombination
  ...
  cc = SET_CMA_ES*(4 + mueff/N) / (N+4 + 2*mueff/N); % time constant for cumulation for C
  cs = (mueff+2) / (N+mueff+5);  % t-const for cumulation for sigma control
  c1 = SET_CMA_ES*2 / ((N+1.3)^2+mueff);    % learning rate for rank-one update of C
  cmu = SET_CMA_ES*min(1-c1, 2 * (mueff-2+1/mueff) / ((N+2)^2+mueff));  % and for rank-mu update
  ...
  % -------------------- Generation Loop --------------------------------
  g = 0;  % the next 40 lines contain the 20 lines of interesting code 
  counteval = 0;
  while g < stopeval
    ...
    %% OMAM
    f_best = arfitness(1);
    f_mean = feval(strfitnessfct, xmean);
    ...
    if f_mean<= stopfitness || max(D) > 1e7 * min(D)
      break;
    end
    ...
    % disp([num2str(g) ': ' num2str(arfitness(1)) ' ' ... 
    %       num2str(sigma*sqrt(max(diag(C)))) ' ' ...
    %       num2str(max(D) / min(D))]);

    % OMAM
    disp([num2str(g) ': ' num2str(f_mean) ' ' ... 
          num2str(sigma*sqrt(max(diag(C)))) ' ' ...
          num2str(max(D) / min(D))]);

    % with long runs, the next line becomes time consuming
    %out.dat = [out.dat; arfitness(1) norm(xmean) sigma D' ]; 
    out.dat = [out.dat; f_mean norm(xmean) sigma max(D) min(D)];   % OMAM
  ...
  %xmin = arx(:, arindex(1)); % Return best point of last iteration.
    xmin = xmean;   % Notice that xmean is expected to be even
                             % better.
  figure; hold off; semilogy(abs(out.dat)); hold on;  % abs for negative fitness
  title('fitness (blue), R (red), sigma(yellow) '); grid on; xlabel('iteration');    
