    ...
    %     subplot(num_var,2,2*current); hold on;
        ...
        switch pc_input.pc_method
            ...
            case 'PSA'
                ...
                GAMMA = 0.88;
                pm_ss = 1-(2-1/GAMMA^2)/(1+pc_input.beta_psa/(sqrt(2/fit.N)*e_vartheta_a_b(0.5,1,0)*(1-pc_input.beta_psa)));
                pc_ss = (1/pc_input.beta_psa-0.5)*8*e_vartheta_a_b(0.5,1,0)^2*pc_input.mu_max/fit.N*(1-GAMMA^2)^2;
                yline(pm_ss,'--');
                yline(pc_ss,'--');
                yscale('log');
                ylim([1e-2,1e2]);
