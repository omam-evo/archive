...
%% To generate Fig 5:
% > execute script with mu variation setting MU={10,100,1000}=pc_input.mu_max=pc_input.mu_min
% > LIST_METHOD loops over all three methods {'APOP', 'PSA', 'PCCMSA'}
% > LIST_CSA = [11] uses CSA 1/sqrt(N)
% > LIST_CONFIG = [2] uses L=10 and beta=1/10 from pc_input/input_v2.m
% > pc_input.pc_on = 0 ensures no active PCS
% > generated plots are R-sigma-mu-dynamics (left) and performance measure (right); 
% > data of performance measures must be extracted manually and inserted into a new figure, such that all curves are merged
% > get_id = [2,4]; selects Sphere with N=100 and Random with N=100
% > data automatically saved in 'prefix_*' folder

%% Describe variation; constant params
prefix = 'fig5';
LIST_METHOD = {'APOP', 'PSA', 'PCCMSA'}; %{'APOP', 'PSA', 'PCCMSA'};
...
get_id = [2,4]; % Sphere: N=100, Random: N=100 selected
fconf = fconf(get_id);
