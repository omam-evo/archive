...
function [y, f_g, r_g, sigma_g, gen, y_g] = csa_es(FIT, N, MU_SWEEP, THETA, Y_0, SIGMA_0, S_PATH_0, C_handle, D_handle, E_chi, ...
    SIGMA_STOP, R_STOP, F_STOP, G, verbose, MODE_CSA, SIGMA_RESCALE, INDEX_VAR)
    ...
    for g = 1:G
        ...
        MU = MU_SWEEP(g);
        LAM = ceil(MU/THETA);
        if SIGMA_RESCALE ~= 0 && g>2
            sigma = sigma*(MU_SWEEP(g)/MU_SWEEP(g-1))^(SIGMA_RESCALE);  
        end
...
%         end
    ...
    if INDEX_VAR==1
        figure; hold on;
        plot(mu_g, '-', 'color', [0.6,0.6,0.6], 'DisplayName','$\mu$')
    else 
        hold on;
    end
    % controller = vecnorm(s_g,2,1)/E_chi;
    % plot(controller, 'b-', 'DisplayName','$||\mathbf{s}||/E_\chi$') 
    plot(r_g, myc{INDEX_VAR}, 'DisplayName',['$R$ ',num2str(SIGMA_RESCALE)])  
    %plot(f_g, 'b')
    set(gca,'Yscale','log');
    %legend;
    %xlabel('g');
