...
%% Calculate progress at r=1 given sigma
R=1;
sph = Sphere;
SIGNZERO = sqrt( sqrt(N*(8*e_vartheta_a_b(MU/LAM,1,0)^2*MU^2 + N)) - N);
sigma0 = 0.9*SIGNZERO*R/N; % estimate sphere st.-st.

phi_norm = sph.phi(0.9*SIGNZERO, e_vartheta_a_b(MU/LAM,1,0), MU, LAM, N, {'medium'});
r = R-phi_norm*R/N;

%% CONFIG 1: initial position and sigma
% POS_LIST = [R]; 
% SIGMA_LIST = [sigma0]; 

%% CONFIG 2: new position and rescaled sigma
% alpha_mu = 1.05;
% POS_LIST = [r]; 
% SIGMA_LIST = [sigma0*sqrt(alpha_mu)]; 

%% CONFIG 3: new pos. same sigma
POS_LIST = [r]; 
SIGMA_LIST = [sigma0]; 

%% SIGMA Regular
FLAG_SIGMA_NORM = 0;
Y_MAT = POS_LIST/sqrt(N)*ones(N,1);

%% SIGMA Norm
% FLAG_SIGMA_NORM = 1;
% SIGMA_LIST = linspace(0,50,11)'; SIGMA_LIST(1) = 1e-5; 
...
for p = 1:NUM_POS
    ...
    for s = 1:NUM_SIG
        ...
        for t = 1:TRIALS
            ...
            [y_opt, F_opt_vec, delta_r_vec, sigma_vec, gen, F_lam] = muComLam_sSA(FIT, N, MU, LAM, Y_0, Y_HAT, SIGMA_0, TAU, SIGMA_STOP, F_STOP, N_G, 0);
        ...
        figure; hold on;
            histogram(F_lam_t, 'BinWidth', 0.2, 'Normalization','pdf');
            [vals,ids] = sort(F_lam_t,1);
            f_med = median(vals(1:MU,:),'all');
            xline(f_med, 'r-', 'LineWidth',1);
        fprintf('R=%f, sigma0=%f, median=%f\n', norm(Y_0), SIGMA_0, f_med);
...
% 
%         %% Phi 2
%         phi_R2_B2(s, p) = phi_R2_reg2norm * sum(phi_2_i(MU, A, ALPHA, sigma, Y_0, e_10, e_11, e_20, {''}),1);
