    ...
    if PLOT_ONLY_PERF==1
        ...
        switch pc_input.pc_method
            ...
            case 'PSA'
                ...
                %% Steady-state
                if pc_input.CSA_MODE==11
                    GAMMA = 0.88; warning('hard-coded gamma')
                elseif pc_input.CSA_MODE==12
                    GAMMA = 0.97; warning('hard-coded gamma')
                else
                    GAMMA = nan;
                end
                beta = pc_input.beta_psa;
                C = sqrt(2/N)*e_vartheta_a_b(THETA,1,0);
                ppm_ss = 1+2*(1-beta)*C*(1/2/GAMMA^2-1)/(beta+C*(1-beta));
                pps_ss_v2 = 1-(2-1/GAMMA^2)/(1+(beta/(1-beta)/C));
                
                yline(ppm_ss, 'k:', 'Alpha',1, 'LineWidth',1);

                if pc_input.mu_min==pc_input.mu_max
                    MU = pc_input.mu_min;
                else
                    MU = nan;
                end
                %phi_ss = sqrt(2*N)*e_vartheta_a_b(THETA,1,0)*(1-GAMMA^2);
                %ppc_ss = (1/beta-0.5)*4*MU/N^2*phi_ss^2;
                ppc_ss = (1/beta-0.5)*MU*8*e_vartheta_a_b(THETA,1,0)^2/N*(1-GAMMA^2)^2;         
                yline(ppc_ss, 'k:', 'Alpha',1, 'LineWidth',1);
                a = 1;
