...
for i = 1:TRIALS
    ...
    if strcmpi(MODE, 'SA')
        [y_opt, F_opt_g, r_g, sigma_g, gen, y_g] = muComLam_sSA_maxInfo(FIT, N, MU, LAM, Y_0, Y_HAT, SIGMA_0, TAU, SIGMA_STOP, R_STOP, N_G, 0);
    end
...
R0 = 1;
y_at_R0 = nan*zeros(N,TRIALS);
for i=1:TRIALS
    [val,id] = min(abs(r_g_t(:,i)-R0));
    y_at_R0(:,i) = y_g_t(:,id,i);
end
figure; hold on;
histogram(y_at_R0, 'normalization', 'pdf', 'NumBins', 50)
m=0;
s=R0/sqrt(N);
x=linspace(m-5*s, m+5*s, 1001);
plot(x,1/s*normpdf(x/s), 'r-');
xlabel('$y_i$');
xlim([-0.5,0.5])
xticks([-0.4:0.2:0.4])
myfigstyle(gcf, 4, 4, 9, 9);
...
figure; hold on;
histogram(y_at_R0, 'normalization', 'pdf', 'NumBins', 25)
m=0;
s=R0/sqrt(N);
x=linspace(m-5*s, m+5*s, 1001);
plot(x,1/s*normpdf(x/s), 'r-');
