function [y_n_rec, F_opt_vec, delta_r_vec, sigma_vec, gen, y_n_g] = muComLam_sSA_maxInfo(fit, N, MU, LAM, Y_0, Y_HAT, SIGMA_0, TAU, SIGMA_STOP, R_STOP, N_G, verbose)
    
    % Values over generations
    F_opt_vec = nan*zeros(N_G+1,1);
    sigma_vec = nan*ones(N_G+1,1);
    delta_r_vec = nan*ones(N_G+1,1);
    y_n_g = nan*ones(N_G+1, N);
