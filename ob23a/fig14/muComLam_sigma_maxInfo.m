...
%         figure; hold on;
%             x = min(y_n_lam(1,:)):0.01:max(y_n_lam(1,:));
%             plot(x, fit(x), 'k--')           
%             plot(y_n_lam(1,:), fit(y_n_lam(1,:)), 'r.')
%             plot(y_n_mu(1,:), fit(y_n_mu(1,:)), 'b.')
%             plot(Y_0(1), fit(Y_0(1)), 'ro')
%             y_rec_temp = mean(y_n_mu, 2)
%             plot(y_rec_temp(1), fit(y_rec_temp(1)), 'ko')
%             title(['mu=',num2str(MU), ' lam=',num2str(LAM), ' sigma=',num2str(SIGMA_G(1)), ' y0=',num2str(Y_0(1))])
%             pause
