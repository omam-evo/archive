...
%          [Var_q] = variance_q_gain_v1(obj, A, alpha, y_mu, y_sigma, sigma); % First results for (1,lam) Q-Gain
    ...
    function [Var_q] = variance_q_gain_simplified(obj, A, alpha, Y, sigma)    
        ...
        Var_q = nan*ones(length(Y), 1);
        for i=1:length(Y)
            Var_q(i) = 2*sigma^4 + 4*Y(i)^2*sigma^2 + A^2/2 + ...
                A^2/2*exp(-(alpha*sigma)^2)*(exp(-(alpha*sigma)^2)*cos(2*alpha*Y(i))-2*cos(alpha*Y(i))^2) + ...
                2*A*alpha*sigma^2*exp(-(alpha*sigma)^2/2)*(alpha*sigma^2*cos(alpha*Y(i))+2*Y(i)*sin(alpha*Y(i)));
        end
        
    end

    %% Approx. D_Q with small Sigma
    function [Var_q] = variance_q_gain_smallSig(obj, A, alpha, Y, sigma)    
        Var_q = 0*Y;
        for i=1:length(Y)
            Var_q(i) = (obj.f_d1(Y(i), alpha, A)*sigma)^2;
        end      
    end
